...
public class AutomovilJugador extends Automovil implements Updatable, Renderable, Collidator {
	...
	public AutomovilJugador(int color, Punto posicion) {
		...
		collider = new Rectangle(posicion.getX() - width / 2, posicion.getY() - height / 2, width, height);
	...
	public void update(double deltaTime) {
		if (directionLeft || directionRight) {
			...
			if ((posicion.getX() + direction * speed * deltaTime > 220)
					&& (posicion.getX() + direction * speed * deltaTime < 490)) {
				setX(posicion.getX() + direction * speed * deltaTime);
				collider.setX(posicion.getX() + direction * speed * deltaTime);
			}
		}
	}
